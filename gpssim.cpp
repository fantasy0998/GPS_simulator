...
int gps_sim(Table *look_up,transfer_parameter *tp, complexf* buff,size_t max_len,int freq_samp,int fc,int tsim, float* dev_i_buff)
{
 ...
 //		}
	...
	checkCuda(cudaMalloc((void**)&dev_parameters, p_n * count * sizeof(float)));//8*count 有什么问题
	checkCuda(cudaMemcpy(dev_noise, noise, sizeof(float) * iq_buff_size * 2, cudaMemcpyHostToDevice));
	checkCuda(cudaMalloc((void**)&dev_para_doub, pd_n * count * sizeof(double)));//8*count 有什么问题
	...
	FILE* fpw;
	fpw = fopen("D:\\testdata\\origindata2.txt", "wt+");
	...
	for (iumd = start; iumd < numd ; iumd++)           //开始每历元，每个通道的循环
	{
		...
		for (i = 0; i < MAX_CHAN; i++)//每次都是重新计算码相位载波相位和步进
		{
			if (tp->chan[i].prn > 0)
			{
				...
				// Signal gain
				//gain[i] = (int)(path_loss*ant_gain*100.0); // scaled by 100 天线增益与距离和仰角都有关系
